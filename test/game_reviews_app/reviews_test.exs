defmodule GameReviewsApp.ReviewsTest do
  use GameReviewsApp.DataCase

  alias GameReviewsApp.Reviews
  ...
  describe "reviews" do
    ...
    test "create_review/1 with valid data creates a review" do
      valid_attrs = %{review_body: "some review_body"}

      assert {:ok, %Review{} = review} = Reviews.create_review(valid_attrs)
      assert review.review_body == "some review_body"
    end
    ...
    test "update_review/2 with valid data updates the review" do
      review = review_fixture()
      update_attrs = %{review_body: "some updated review_body"}

      assert {:ok, %Review{} = review} = Reviews.update_review(review, update_attrs)
      assert review.review_body == "some updated review_body"
    end
